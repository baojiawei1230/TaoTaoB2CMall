seajs.use([
  ...
  ], function (globalInit,lazyload,switchable,tips,dialog,placeholder,scrollbar,drag,someMoreRecommend){//FE 模块加载 start
  globalInit();
...
function consigneeInfo(){
  ...
  var phone = selectDiv.find(".addr-tel").html();
    var info="<p>寄送至："+address+"</p><p>收货人："+name+"&nbsp;&nbsp;"+phone+"</p>";
...
function showWhiteBar() {
  try {
    jQuery.getJSON("//baitiao.jd.com/account/query?callback=?", function(data) {
      if (data == null) {
        return;
      }
      if (data.result != null && data.result.isSuccess && (data.status == 2 || data.status == 3)) {
        //$(".whiteBarSpanClass").removeClass("hide");
        $("#cod_bt").removeClass("hide");
        $("#cod").addClass("hide");
      } else {
        $("#cod_bt").addClass("hide");
 ...
 * 调用后台接口获取可用优惠券数量
 */
function showCouponAvailableNumByRpc(){
      var url = OrderAppConfig.AsyncDomain + "/coupon/getAvailableNum.action";
      jQuery.ajax({
        type : "POST",
        dataType : "text",
        url : url,
        data :addFlowTypeParam(),
        async : true,
        cache : false,
...
function flushOrderPrice(orderPrice, isFlushSkuList) {
  ...
  $("#tooltip-box05 .ftx-01").each(function(){
...
// 预售结算页 有message提示
if ($("#isPresale").val() == "true") {
  $("#submit_check_info_message").html("<span>定金恕不退换，请同意支付定金</span>").show();
  ...
  if(noStockType == "600158"){
    ...
    try{
      var firstSku = g_outSkus.split(",")[0];
        someMoreRecommend(firstSku,'900011','trade-nostock-recommendation-render', function(hasData){
          if(!hasData){
