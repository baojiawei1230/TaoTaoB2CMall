...
function delete_Consignee(id) {
  ...
  var actionUrl ="http://order.taotao.com/service/consignee/deleteConsignee/"+id;
	param = addFlowTypeParam(param);
...
function use_NewConsignee() {
  ...
  $('body').dialog({
    ...
    height:290,
    type:'iframe',
    source:"http://www.taotao.com/service/consignee/toConsignee?consigneeId="+consigneeId
  });
...
function save_Pay(type) {
	...
	alert(param);
	//传递支付类型
	$("input[name='paymentType']").val(payId);
...
function edit_Invoice() {
	$('body').dialog({
		...
		source: "http://www.taotao.com/order/invoice.html"
	});
}
...
function openEditConsigneeDialog(id){
	$('body').dialog({
		...
		source:"http://www.taotao.com/service/consignee/toConsignee?consigneeId="+id
	});
